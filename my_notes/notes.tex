...
\begin{description}
	...
	\begin{itemize}
		...
		\item Note: Everywhere below, we can replace $X_{i}$ with $\phi(X_{i})$
			...
			\State $f(z) \leftarrow w^{T}x$
			\EndWhile
			\end{algorithmic}
			\end{algorithm*}
		\item Kernelize with $w=X^{T}a$; $X_{i}\cdot w = (XX^{T}a)_{i} = (Ka)_{i}$.
					\begin{algorithm*}
			\caption{Dual Perceptron algorithm}
			\begin{algorithmic}
			\State $ a = [y_{1}, 0, \dots, 0]^{T}$
			\State $K_{ij} = K(X_{i}, X_{j}) \ \forall i,j$
			\While {some $y_{i}(Ka)_{i} \cdot w < 0$}
			\State $a \leftarrow a + \epsilon y_{i}$
			\EndWhile
			\While{still have test points $z$}
			\State $f(z) \leftarrow \sum_{j=1}^{n} a_{j}K(X_{j},z)$
			\EndWhile
			\end{algorithmic}
			\end{algorithm*}
